import{a as m,S as M,A as _,b as P,N as I,P as O,K as T}from"./assets/vendor-DD8E1I8W.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const k of i.addedNodes)k.tagName==="LINK"&&k.rel==="modulepreload"&&o(k)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();const v=document.querySelector(".mobile-menu"),D=document.querySelector(".burger-menu.bth-open"),G=document.querySelector(".mobile-menu .close-btn"),K=document.querySelectorAll(".mob-menu-link, .mob-menu-button"),V=document.querySelectorAll(".list-mob, .mob-menu"),$=()=>v.classList.toggle("is-open"),S=()=>document.body.classList.toggle("is-scroll-disabled");v.addEventListener("click",e=>{e.target===v&&b()});document.addEventListener("keydown",e=>{e.key==="Escape"&&v.classList.contains("is-open")&&b()});const b=()=>{$(),S()};D.addEventListener("click",()=>{$(),S()});G.addEventListener("click",()=>{$(),S()});K.forEach(e=>{e.addEventListener("click",b)});V.forEach(e=>{e.addEventListener("click",b)});const q="https://paw-hut.b.goit.study/api",F=async({page:e=1,limit:t=9,categoryId:s=""}={})=>{try{return(await m.get(`${q}/animals`,{params:{page:e,limit:t,...s&&{categoryId:s}}})).data}catch(o){throw console.error("API Error (getAnimals):",o),o}},U=async()=>{try{return(await m.get(`${q}/categories`)).data}catch(e){throw console.error("API Error (getCategories):",e),e}},Z=async e=>{try{return(await m.post(`${q}/orders`,e)).data}catch(t){throw console.error("API Error (createOrder):",t),t}},g=document.querySelector(".backdrop"),J=document.querySelector(".modal-close"),Q=document.querySelector(".modal-form");let j=null;function X(e){j=e,g.classList.remove("visually-hidden"),document.body.style.overflow="hidden",document.addEventListener("keydown",H)}function L(){g.classList.add("visually-hidden"),document.body.style.overflow="",document.removeEventListener("keydown",H)}function H(e){e.key==="Escape"&&L()}J.addEventListener("click",L);g.addEventListener("click",e=>{e.target===g&&L()});Q.addEventListener("submit",async e=>{e.preventDefault();const{name:t,phone:s,comment:o}=e.target.elements,n={name:t.value,phone:s.value,animalId:j,comment:o.value};try{await Z(n),M.fire({title:"Success!",icon:"success",timer:1e3,draggable:!0}),e.target.reset(),L()}catch{M.fire({icon:"error",title:"Oops...",text:"Something went wrong!"})}});function Y(e,t){e.innerHTML=`
